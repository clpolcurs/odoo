...
class Binary(http.Controller):
    @http.route(
        "/web/binary/vote_list_export_result/<vote_list_ids>",
        type="http",
        auth="user",
        website=True,
    )
    def export_vote_list_result_as_pdf(self, **kwargs):
        ...
        for vote_list in vote_lists:
            ...
            title = Paragraph(f"{vote_list.name} - Kết quả".upper(), title_style)
            elements.append(title)

            # Create table data
            table_data = [[Paragraph(item, header_style) for item in
                           ["STT", "Họ Tên", "Chức vụ/ Chức danh", "Số phiếu bầu", "Tỷ lệ"]]]
            vote_details = vote_list.get_vote_details_for_result_export()
            table_data.extend(
                [
        ...
        file_name = f"{vote_lists[0].name} - Kết quả".encode('utf-8').decode('latin-1', 'ignore')
